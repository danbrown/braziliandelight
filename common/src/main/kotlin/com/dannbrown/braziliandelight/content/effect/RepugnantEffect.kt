...
class RepugnantEffect : MobEffect(MobEffectCategory.HARMFUL, 0X577D4D) {
  override fun applyEffectTick(entity: LivingEntity, amplifier: Int) {
    if (!entity.commandSenderWorld.isClientSide) {
      ...
      val range = AABB(
        entity.x - radiusHorizontal,
        entity.y - radiusVertical,
        entity.z - radiusHorizontal,
        entity.x + radiusHorizontal,
        entity.y + radiusVertical,
        entity.z + radiusHorizontal
      )
        ...
        if (mob != null) {
          ...
          if (Mth.abs(x2.toInt()) < radiusHorizontal && Mth.abs(z2.toInt()) < radiusVertical * 2 && Mth.abs(y2.toInt()) < radiusHorizontal) {
            ...
            if ((vec3d != null && wb.isWithinBounds(
              ...
              )) && this.distanceToSqr(entity, vec3d.x, vec3d.y, vec3d.z) >= distanceToSqr(entity, mob.position())
            ) {
