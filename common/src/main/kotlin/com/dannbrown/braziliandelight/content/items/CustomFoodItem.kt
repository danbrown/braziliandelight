...
open class CustomFoodItem(
  ...
  override fun appendHoverText(
    ...
    if (this.hasCustomTooltip) {
      val textEmpty: MutableComponent = Component.translatable(ModContent.MOD_ID + ".tooltip.$this")
    ...
    if (effectList.isEmpty()) {
      ...
      for (effectPair in effectList) {
        ...
        if (attributeMap.isNotEmpty()) {
          for ((key, rawModifier) in attributeMap) {
            val modifier = AttributeModifier(
              rawModifier.name,
              effect.getAttributeModifierValue(instance.amplifier, rawModifier),
              rawModifier.operation
            )
        ...
        if (instance.amplifier > 0) {
          iformattabletextcomponent = Component.translatable(
        ...
        if (instance.duration > 20) {
          iformattabletextcomponent = Component.translatable(
            "potion.withDuration",
            iformattabletextcomponent,
            MobEffectUtil.formatDuration(instance, durationFactor)
          )
        }
