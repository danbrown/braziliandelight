...
import com.dannbrown.braziliandelight.init.ModEffects
...
class RepugnantArrow : AbstractArrow {
  ...
  constructor(level: Level, livingEntity: LivingEntity) : super(
    ModEntityTypes.REPUGNANT_ARROW.get(),
    livingEntity,
    level
  )

  constructor(level: Level, px: Double, py: Double, pz: Double) : super(
    ModEntityTypes.REPUGNANT_ARROW.get(),
    px,
    py,
    pz,
  ...
  override fun getPickupItem(): ItemStack {
    return ItemStack(ModItems.REPUGNANT_ARROW.get())
  }
  ...
  override fun doPostHurtEffects(livingEntity: LivingEntity) {
    ...
    val repugnantEffect = MobEffectInstance(ModEffects.REPUGNANT.get(), 400, 1)
    ...
    for (entity in entities) {
      if (entity != null) {
        ...
        if (Mth.abs(x2.toInt()) < 12 && Mth.abs(z2.toInt()) < 6 && Mth.abs(y2.toInt()) < 12) {
          ...
          if ((vec3d != null && wb.isWithinBounds(
              BlockPos(
                entity.xOld.toInt(),
                entity.yOld.toInt(),
                entity.zOld.toInt()
              )
            )) && this.distanceToSqr(vec3d.x, vec3d.y, vec3d.z) >= this.distanceToSqr(entity)
          ) {
